...
export default class LoginBackEnd extends Component {
  constructor(props) {
    super(props)
    this.state = {
      email : '',
  ...
  onSubmit = (event) => {
    ...
    .then(res => {
      if (res.status === 200) {
        ...
        this.props.value.setActiveBackEndUser(this.state.email, history, destination)
      } else {
